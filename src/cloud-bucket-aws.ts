import { Bucket, File, DownloadEvents, buildFullDestPath, extractPrefixAndGlob } from "./cloud-bucket-base";
import { readFile, createWriteStream } from 'fs-extra-plus';
...
class AwsBucket implements Bucket {
	...
	async copy(pathOrGlob: string, destDir: string | File): Promise<void> {
		...
		const destPathDir = (typeof destDir === 'string') ? destDir : destDir.name;
	...
	async download(pathOrGlob: string, localDir: string, on?: DownloadEvents | undefined): Promise<File[]> {
		const awsFiles = await this.listAwsFiles(pathOrGlob);
		const files: File[] = [];

		for (let af of awsFiles) {
			const remotePath = af.Key!;
			...
			try {
				...
				const localWriteStream = createWriteStream(destination);
				...
				await writePromise;
				// FIXME: needs to give the size (either from localFile, or from bytes streamed)
				files.push({ bucket: this, name: remotePath, size: -1 });
	...
	async upload(localPath: string, destPath: string): Promise<File> {
		...
		try {
			...
			return { bucket: this, name: fullDestPath, size: localFileData.length };
		} catch (ex) {
			process.stdout.write(` - FAIL - ABORT - Cause: ${ex}\n`);
			throw ex;
		}
