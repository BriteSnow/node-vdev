...
class GcpBucket implements Bucket {
	...
	async download(pathOrGlob: string, localDir: string, on?: DownloadEvents | undefined): Promise<File[]> {
		...
		for (let gf of googleFiles) {
			const baseName = Path.basename(gf.name);
			const destination = `${localDir}${baseName}`;

			process.stdout.write(`Downloading gs://${this.googleBucket.name}/${gf.name} to ${destination}`);
			try {
				await gf.download({ destination });
				process.stdout.write(` - DONE\n`);
			} catch (ex) {
				process.stdout.write(` - FAIL - ABORT - Cause: ${ex}\n`);
				throw ex;
			}
			const file = this.toFile(gf);
